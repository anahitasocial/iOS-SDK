...
- (id)transformValue:(id)value atKeyPath:(NSString *)keyPath toType:(Class)destinationType {
    ...
    SEL selector = @selector(initWithMappableValue:);
    
    if ( class_respondsToSelector(destinationType, selector) ) {    
        id instance = [[destinationType alloc] performSelector:selector withObject:value];
        return instance;
    }
