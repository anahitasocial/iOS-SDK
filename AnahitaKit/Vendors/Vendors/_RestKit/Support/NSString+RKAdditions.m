...
- (NSDictionary*)queryParametersUsingArrays:(BOOL)shouldUseArrays encoding:(NSStringEncoding)encoding {
    ...
    while (![scanner isAtEnd]) {
        ...
        NSMutableArray* kvPair = [NSMutableArray arrayWithArray:[pairString componentsSeparatedByString:@"="]];
        BOOL isArray = shouldUseArrays;
        if ( kvPair.count == 2 ) {
            NSString *key = [kvPair objectAtIndex:0];
            if ( [key rangeOfString:@"[]"].location != NSNotFound ) {
                key = [key stringByReplacingOccurrencesOfString:@"[]" withString:@""];
                [kvPair replaceObjectAtIndex:0 withObject:key];
                isArray = YES;
            }
        }
        if (!isArray) {
